...
module Part2.FlatF where

import Part1.Intro2 hiding (main)
import qualified Part2.PushNegF as Neg hiding (main)
...
main = do
  ...
  print tf4_norm_view
  print tf4_norm_eval
  if tf4_eval == tf4_norm_eval
    then return ()
    else error "Normalization"
  if (view . norm $ tf4_norm) == tf4_norm_view
    then return ()
    else error "Normalization"
