...
module Part2.PushNegFI where

-- Explain the imports:
-- The final representation of Exp

import Part1.Intro1 (Exp (..)) -- import the Exp data type
import Part1.Intro2 hiding (main)
import qualified Part2.PushNegI as I -- use the initial push_neg as it was
...
tf1n_norm :: ExpSYM c => c
tf1n_norm = push_neg (neg tf1)

-- see the result
tf1n_norm_view :: String
...
tf1nn_norm :: ExpSYM c => c
tf1nn_norm = push_neg (neg tf1n_norm)

tf1nn_norm_view :: String
...
data ExpSYMDict repr = ExpSYMDict
  { lit_dict :: Int -> repr,
    neg_dict :: repr -> repr,
...
main :: IO ()
main = do
  print Part2.PushNegFI.tf1_view
  print tf1_norm_view
  print tf1n_norm_view
  if tf1_norm_view == tf1nn_norm_view
    then return ()
    else error "Double neg"
  print tf1nn_norm_view
  if eval tf1 == tf1_norm_eval
    then return ()
    else error "Normalization"
  if eval tf1 == - tf1n_norm_eval
    then return ()
    else error "Normalization"
  print tf1'_eval
