...
fromTree (Node "Add" [e1, e2]) = liftA2 add (fromTree e1) (fromTree e2)
...
check_consume :: (t -> IO ()) -> Either ErrMsg t -> IO ()
...
dup_consume :: (Show a, ExpSYM t, ExpSYM b) => (t -> a) -> (t, b) -> IO b
...
fromTreeExt self (Node "Add" [e1, e2]) = liftA2 add (self e1) (self e2)
...
-- * Does each evaluation of tf1' re-parse tf1_tree?
